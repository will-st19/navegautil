...
function obterProximoCiclo(estacao, dataBase) {
    ...
    // Criamos as datas usando números (ano, mes, dia) para garantir Horário Local
    switch (estacao) {
        case "verao":
            inicio = new Date(ano - 1, 11, 21); // 21 Dez anterior
            fim = new Date(ano, 2, 19, 23, 59, 59, 999); // 19 Mar às 23:59:59
            
            if (dataBase > fim) {
                inicio = new Date(ano, 11, 21);
                fim = new Date(ano + 1, 2, 19, 23, 59, 59, 999);
            }
            break;

        case "outono":
            inicio = new Date(ano, 2, 20); 
            fim = new Date(ano, 5, 20, 23, 59, 59, 999);
            break;

        case "inverno":
            inicio = new Date(ano, 5, 21);
            fim = new Date(ano, 8, 22, 23, 59, 59, 999);
            break;

        case "primavera":
            inicio = new Date(ano, 8, 23);
            fim = new Date(ano, 11, 20, 23, 59, 59, 999);
            break;
    }

    // Ajuste para as outras estações caso já tenham passado
